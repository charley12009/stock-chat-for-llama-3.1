...
# Function to create and display the API key input window
def get_api_key():
    def submit_key():
        global client
        ...
        if api_key:
            ...
            client = Groq()  # Initialize the client with the new API key
            api_key_window.destroy()
            main_application()  # Start the main application

    api_key_window = tk.Tk()
    api_key_window.title("Enter API Key")
    api_key_window.geometry("300x150")

    label = tk.Label(api_key_window, text="Please enter your GROQ Cloud API key:")
    label.pack(pady=10)

    api_key_entry = tk.Entry(api_key_window, width=40)
    api_key_entry.pack(pady=5)

    submit_button = tk.Button(api_key_window, text="提交", command=submit_key)
...
def main_application():
    global date_shown  # Reference the global variable
    root = tk.Tk()
...
# Start the API key input window
get_api_key()
